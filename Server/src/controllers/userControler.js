...
module.exports = {
    ...
    getFollower(request, response) {
        const { id } = request.params
        connection('usuario_usuario').where('usuario_seguido_id', id).select('usuario_id').then(res => {
            return response.json(res);
        }).catch(error => {
            response.json({ err: error, msg: error.toString() });
        })
    ...
    getFollowing(request, response) {
        const { id } = request.params
        connection('usuario_usuario').where('usuario_id', id).select('usuario_seguido_id').as('usuario_id').then(res => {
            return response.json(res);
        }).catch(error => {
            response.json({ err: error, msg: error.toString() });
        })
